...
internal class RadiusOfOperationMilesV4Endpoint: EndpointBase, IEndpoint
{
    public RadiusOfOperationMilesV4Endpoint(IVendorAdapter vendorAdapter, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs)
    : base(vendorAdapter, environmentName, maxDegreeOfParallelism, maxRetries, minRetryDelayMs) {}

    public override string ResourceName => "radius-of-operation";
        ...
        var requests = carrierEldList.Select(c =>
            new Request(_vendorAdapter, resourceName: ResourceName, resourceVersion: ResourceVersion, pagination: paginatedRequestSettings,
                        queryParameters: new Dictionary<string, string>
                        {
                            ...
                            ["startTime"] = startUtc.ToString("yyyy-MM-dd'T'HH:mm:ss'Z'"),
                            ["endTime"] = endUtc.ToString("yyyy-MM-dd'T'HH:mm:ss'Z'")
                        })
        ).ToList();
