...
internal class GpsMilesV4Endpoint: EndpointBase, IEndpoint
{
    public GpsMilesV4Endpoint(IVendorAdapter vendorAdapter, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs)
    : base(vendorAdapter, environmentName, maxDegreeOfParallelism, maxRetries, minRetryDelayMs) {}

    public override string ResourceName => "enriched-data/gps-miles";
    ...
    public async Task<List<FetchResult>> Fetch(IngestionRun? ingestionRun,List<FetchResult> carriers, DateTimeOffset startUtc, DateTimeOffset endUtc, int pageSize = 500, int? maxNrPagesBeforeAbort = null
