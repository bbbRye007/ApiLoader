...
internal class SubscriptionsV4Endpoint: EndpointBase, IEndpoint
{
    internal SubscriptionsV4Endpoint(IVendorAdapter vendorAdapter, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs)
    : base(vendorAdapter, environmentName, maxDegreeOfParallelism, maxRetries, minRetryDelayMs) {}

    public override string ResourceName => "subscriptions";
    ...
    internal async Task<List<FetchResult>> Fetch(IngestionRun? ingestionRun, int pageSize, int? maxNrPagesBeforeAbort = null, CancellationToken cancellationToken = default)
    {
