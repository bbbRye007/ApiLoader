...
internal class RiskScoresV4Endpoint: EndpointBase, IEndpoint
{
    public RiskScoresV4Endpoint(IVendorAdapter vendorAdapter, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs)
    : base(vendorAdapter, environmentName, maxDegreeOfParallelism, maxRetries, minRetryDelayMs) {}
    
    public override string ResourceName => "risk-scores";
    ...
    public async Task<List<FetchResult>> Fetch(IngestionRun? ingestionRun, List<FetchResult> carriers, int pageSize, int? maxNrPagesBeforeAbort = null, CancellationToken cancellationToken = default)
    {
