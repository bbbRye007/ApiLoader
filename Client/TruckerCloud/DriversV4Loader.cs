...
public class DriversV4Loader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public DriversV4Loader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
    ...
    {
        var endpoint = new DriversV4Endpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);
