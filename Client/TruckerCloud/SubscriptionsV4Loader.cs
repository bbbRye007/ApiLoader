...
public class SubscriptionsV4Loader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public SubscriptionsV4Loader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
    ...
    public async Task<List<FetchResult>> Load(int pageSize = 500, int? maxNrPagesBeforeAbort = null, bool saveResults = true, CancellationToken cancellationToken = default)
    {
        var endpoint = new SubscriptionsV4Endpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);
