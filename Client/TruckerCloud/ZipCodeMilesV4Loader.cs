...
public class ZipCodeMilesV4Loader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public ZipCodeMilesV4Loader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var endpoint = new ZipCodeMilesV4Endpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);        

        DateTimeOffset? warterMarkEndUtc = await GetEndUtcFromWatermark_Async(cancellationToken); 
