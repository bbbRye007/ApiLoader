...
public class SafetyEventsV5Loader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public SafetyEventsV5Loader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var endpoint = new SafetyEventsV5Endpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);        
        ...
        {
            results = await endpoint.Fetch(IngestionRun, carriers, startUtc, endUtc, pageSize, maxNrPagesBeforeAbort, bodyParamsJson, cancellationToken).ConfigureAwait(false);
