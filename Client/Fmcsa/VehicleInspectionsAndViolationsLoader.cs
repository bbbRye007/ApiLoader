...
public class VehicleInspectionsAndViolationsLoader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public VehicleInspectionsAndViolationsLoader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
    ...
    {
        var endpoint = new VehicleInspectionsAndViolationsEndpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);
