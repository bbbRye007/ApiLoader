...
public class CarrierAllHistoryLoader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public CarrierAllHistoryLoader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
    ...
    {
        var endpoint = new CarrierAllHistoryEndpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);
