...
public class SmsInputViolationLoader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public SmsInputViolationLoader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
    ...
    {
        var endpoint = new SmsInputViolationEndpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);
