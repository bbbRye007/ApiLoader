...
public class SmsInputCrashLoader: EndpointLoaderBase
{

    [SetsRequiredMembers]
    public SmsInputCrashLoader(IVendorAdapter vendorAdapter, BlobContainerClient containerClient, string environmentName, int maxDegreeOfParallelism, int maxRetries, int minRetryDelayMs) 
    ...
    {
        var endpoint = new SmsInputCrashEndpoint(VendorAdapter, EnvironmentName, MaxDegreeOfParallelism, MaxRetries, MinRetryDelayMs);
        InitRun(endpoint);
